...
outworkbook = xlsxwriter.Workbook("C:\\Users\\amitk\\OneDrive\\Desktop\\amit2.xlsx")
outsheet = outworkbook.add_worksheet()

element = matrix([[1, 2, 3, 4],
                  [4, 3, 5, 6]])
nodeCordinate = matrix([[0, 0.0],
                        [10, 0],
                        [10, 10.0],
                        [0, 10.0],
                        [10, 20],
                        [0, 20.0]])
doff = matrix([[0, 0], [0, 0], [1, 1], [1, 1], [0, 1], [0, 1]])
...
utPlusDeltglobal = matrix(
    [[0], [0], [0], [0], [.170], [.34], [.0420], [.11], [0], [.030], [0], [.15]])
...
for t in range(0, maxTime, 1):  ####
    ...
    reg = matrix([[0], [0], [0], [0], [0], [0], [0], [0], [0], [-load], [0], [-load]])  # here mey be problem
    ...
    for i in range(0, noNode, 1):
        ut1global[i] = utglobal[2 * i, 0]
        ut2global[i] = utglobal[(2 * i) + 1, 0]
    ...
    for ele in range(0, noEle, 1):  #####
        ...
        ng = matrix(
            [2 * eleNode1 - 2, 2 * eleNode1 - 1, 2 * eleNode2 - 2, 2 * eleNode2 - 1, 2 * eleNode3 - 2, 2 * eleNode3 - 1,
             2 * eleNode4 - 2, 2 * eleNode4 - 1])
        # print(ng)
        ...
        UtVtWt = matrix([U1t, V1t, U2t, V2t, U3t, V3t, U4t, V4t])
        #print(UtVtWt)
        ...
        for m in range(0, 2, 1):
            for n in range(0, 2, 1):
                ...
                Jmatrix = matrix([[Jstar[0, 0], Jstar[0, 1], 0, 0],
                                  [0, 0, Jstar[0, 0], Jstar[0, 1]],
                                  [Jstar[1, 0], Jstar[1, 1], 0, 0],
                                  [0, 0, Jstar[1, 0], Jstar[1, 1]]])

                srMatrix = matrix([[DNr[0, 0], 0, DNr[0, 1], 0, DNr[0, 2], 0, DNr[0, 3], 0],
                                   [DNs[0, 0], 0, DNs[0, 1], 0, DNs[0, 2], 0, DNs[0, 3], 0],
                                   [0, DNr[0, 0], 0, DNr[0, 1], 0, DNr[0, 2], 0, DNr[0, 3]],
                                   [0, DNs[0, 0], 0, DNs[0, 1], 0, DNs[0, 2], 0, DNs[0, 3]]])
                ...
                bma = matrix([[dUdXdVdXdWdXdUdYdVdYdWdY[0, 0] + 1, dUdXdVdXdWdXdUdYdVdYdWdY[1, 0], 0, 0],
                              [0, 0, dUdXdVdXdWdXdUdYdVdYdWdY[2, 0], dUdXdVdXdWdXdUdYdVdYdWdY[3, 0] + 1],
                              [dUdXdVdXdWdXdUdYdVdYdWdY[2, 0], dUdXdVdXdWdXdUdYdVdYdWdY[3, 0] + 1,
                               dUdXdVdXdWdXdUdYdVdYdWdY[0, 0] + 1,
                               dUdXdVdXdWdXdUdYdVdYdWdY[1, 0]]])
                ...
                epsXXt = dUdXdVdXdWdXdUdYdVdYdWdY[0, 0] + 1 / 2 * pow(dUdXdVdXdWdXdUdYdVdYdWdY[0, 0], 2) + 1 / 2 * pow(
                    dUdXdVdXdWdXdUdYdVdYdWdY[1, 0], 2)
                epsYYt = dUdXdVdXdWdXdUdYdVdYdWdY[3, 0] + 1 / 2 * pow(dUdXdVdXdWdXdUdYdVdYdWdY[2, 0], 2) + 1 / 2 * pow(
                    dUdXdVdXdWdXdUdYdVdYdWdY[3, 0], 2)
                epsXYt = 1 / 2 * (dUdXdVdXdWdXdUdYdVdYdWdY[1, 0] + dUdXdVdXdWdXdUdYdVdYdWdY[2, 0] +
                                  dUdXdVdXdWdXdUdYdVdYdWdY[0, 0] * dUdXdVdXdWdXdUdYdVdYdWdY[2, 0] +
                                  dUdXdVdXdWdXdUdYdVdYdWdY[1, 0] * dUdXdVdXdWdXdUdYdVdYdWdY[3, 0])
                ...
                CnonLin = matrix([[StXXStYYStXY[0, 0], 0, StXXStYYStXY[1, 0], 0],
                                  [0, StXXStYYStXY[0, 0], 0, StXXStYYStXY[1, 0]],
                                  [StXXStYYStXY[1, 0], 0, StXXStYYStXY[2, 0], 0],
                                  [0, StXXStYYStXY[1, 0], 0, StXXStYYStXY[2, 0]]])
    ...
    while iterationParameter == 0:
        ...
        for i in range(0, noNode, 1):
            utPlusDelt1global[i] = utPlusDeltglobal[2 * i, 0]
            utPlusDelt2global[i] = utPlusDeltglobal[(2 * i) + 1, 0]
        ...
        for ele in range(0, noEle, 1):
            ...
            eleNode3 = element[ele, 2]
            eleNode4 = element[ele, 3]
            ng = matrix([2 * eleNode1 - 2, 2 * eleNode1 - 1, 2 * eleNode2 - 2, 2 * eleNode2 - 1, 2 * eleNode3 - 2,
                         2 * eleNode3 - 1,
                         2 * eleNode4 - 2, 2 * eleNode4 - 1])
            ...
            UtVtWttPlusDelt = matrix(
                [U1tPlusDelt, V1tPlusDelt, U2tPlusDelt, V2tPlusDelt, U3tPlusDelt,
                 V3tPlusDelt, U4tPlusDelt, V4tPlusDelt])
            ...
            for m in range(0, 2, 1):
                for n in range(0, 2, 1):
                    ...
                    bmatPlusDelt = matrix(
                        [[dUdXdVdXdWdXdUdYdVdYdWdYtPlusDelt[0, 0] + 1, dUdXdVdXdWdXdUdYdVdYdWdYtPlusDelt[1, 0], 0, 0],
                         [0, 0, dUdXdVdXdWdXdUdYdVdYdWdYtPlusDelt[2, 0], dUdXdVdXdWdXdUdYdVdYdWdYtPlusDelt[3, 0] + 1],
                         [dUdXdVdXdWdXdUdYdVdYdWdYtPlusDelt[2, 0], dUdXdVdXdWdXdUdYdVdYdWdYtPlusDelt[3, 0] + 1,
                          dUdXdVdXdWdXdUdYdVdYdWdYtPlusDelt[0, 0] + 1,
                          dUdXdVdXdWdXdUdYdVdYdWdYtPlusDelt[1, 0]]])
                    JmatrixsrMatrix = Jmatrix.dot(srMatrix)
                    ...
                    epsXXtPlusDelt = dUdXdVdXdWdXdUdYdVdYdWdYtPlusDelt[0, 0] + 1 / 2 * pow(
                        dUdXdVdXdWdXdUdYdVdYdWdYtPlusDelt[0, 0], 2) + 1 / 2 * pow(
                        dUdXdVdXdWdXdUdYdVdYdWdYtPlusDelt[1, 0], 2)
                    epsYYtPlusDelt = dUdXdVdXdWdXdUdYdVdYdWdYtPlusDelt[3, 0] + 1 / 2 * pow(
                        dUdXdVdXdWdXdUdYdVdYdWdYtPlusDelt[2, 0], 2) + 1 / 2 * pow(
                        dUdXdVdXdWdXdUdYdVdYdWdYtPlusDelt[3, 0], 2)
                    epsXYtPlusDelt = 1 / 2 * (
                                dUdXdVdXdWdXdUdYdVdYdWdYtPlusDelt[1, 0] + dUdXdVdXdWdXdUdYdVdYdWdYtPlusDelt[2, 0] +
                                dUdXdVdXdWdXdUdYdVdYdWdYtPlusDelt[0, 0] * dUdXdVdXdWdXdUdYdVdYdWdYtPlusDelt[2, 0] +
                    ...
                    CnonLintPlusDelt = matrix([[StXXStYYStXYtPlusDelt[0, 0], 0, StXXStYYStXYtPlusDelt[1, 0], 0],
                                               [0, StXXStYYStXYtPlusDelt[0, 0], 0, StXXStYYStXYtPlusDelt[1, 0]],
                                               [StXXStYYStXYtPlusDelt[1, 0], 0, StXXStYYStXYtPlusDelt[2, 0], 0],
                                               [0, StXXStYYStXYtPlusDelt[1, 0], 0, StXXStYYStXYtPlusDelt[2, 0]]])
